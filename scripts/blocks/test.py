...
def copy_range_with_styles(src_ws, dst_ws, src_range, dst_cell):
    """
    Versión corregida que maneja correctamente celdas combinadas
    """
    try:
        ...
        for row in range(src_start_row, src_end_row + 1):
            for col in range(src_start_col, src_end_col + 1):
                ...
                # Saltar celdas combinadas (ya las manejamos antes)
...
if __name__ == "__main__":
    try:
        ...
        src_ws = wb['Reporte']
        dst_ws = wb['Hoja2'] if 'Hoja2' in wb.sheetnames else wb.create_sheet('Hoja2')

        if copy_range_with_styles(src_ws, dst_ws, 'B1:AQ11', 'F1'):
            output_path = r'C:\Users\Duban Serrano\Desktop\Nuevo Hoja de cálculo de Microsoft Excel.xlsx'
            wb.save(output_path)
            print(f"¡Éxito! Archivo guardado en: {output_path}")
        else:
            print("Error durante la copia")
